  ...
  describe '#add' do
    ...
    context 'When the string contains negative numbers' do
      it 'Should raises an exception' do
        ...
        expect do
          calculator.add
        end.to raise_error(Errors::NegativeNumberError, 'Negative numbers are not allowed: -4')
      end
    end
    ...
    context 'When the string contains invalid input' do
      it 'Should raises an exception' do
        calculator = StringCalculator.new("1,\n")
        expect { calculator.add }.to raise_error(Errors::InvalidInputError, 'Invalid input error')
      end
